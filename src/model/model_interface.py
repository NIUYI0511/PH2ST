...
class  ModelInterface(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        #---->Forward
        if self.model_name in ['TRIPLEX','his_to_gene','his_2st','hgg_ep','PH2ST']:
    ...
    def test_step(self, batch, batch_idx):
        ...
        # batch_size = results_dict['logits'].size(0)
        # num_select = int(batch_size * 0.1)  # 选取 10% 样本
        
        # #---->Loss
        # logits = results_dict['logits'][num_select:]
        # label = batch['label'][num_select:]
        logits = results_dict['logits']
        label = batch['label']
        
        test_metric = self.test_metrics(logits, label)
        print(test_metric['test_PearsonCorrCoef'].mean())
        if os.path.exists(f"{self.config.DATA.output_path}/idx_top.npy"):
